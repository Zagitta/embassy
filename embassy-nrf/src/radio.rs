...
impl From<Ieee802154Channel> for Frequency {
    fn from(chan: Ieee802154Channel) -> Self {
        // As per 6.20.12.2 Operating frequencies
        Self(match chan {
            Ieee802154Channel::C11 => 2405,
            Ieee802154Channel::C12 => 2410,
            Ieee802154Channel::C13 => 2415,
            Ieee802154Channel::C14 => 2420,
            Ieee802154Channel::C15 => 2425,
            Ieee802154Channel::C16 => 2430,
            Ieee802154Channel::C17 => 2435,
            Ieee802154Channel::C18 => 2440,
            Ieee802154Channel::C19 => 2445,
            Ieee802154Channel::C20 => 2450,
            Ieee802154Channel::C21 => 2455,
            Ieee802154Channel::C22 => 2460,
            Ieee802154Channel::C23 => 2465,
            Ieee802154Channel::C24 => 2470,
            Ieee802154Channel::C25 => 2475,
            Ieee802154Channel::C26 => 2480,
        })
    }
}
...
impl From<PreambleLength> for PLEN_A {
    fn from(pl: PreambleLength) -> Self {
        match pl {
            PreambleLength::P8bit => Self::_8BIT,
            PreambleLength::P16bit => Self::_16BIT,
            PreambleLength::P32bitZero => Self::_32BITZERO,
            PreambleLength::PLongRange => Self::LONGRANGE,
        }
    }
}
...
pub enum CrcSkipAddressMode {
    Include,
    Skip,
    Ieee802154,
}

impl From<CrcSkipAddressMode> for SKIPADDR_A {
    fn from(m: CrcSkipAddressMode) -> Self {
        match m {
            CrcSkipAddressMode::Include => Self::INCLUDE,
            CrcSkipAddressMode::Skip => Self::SKIP,
            CrcSkipAddressMode::Ieee802154 => Self::IEEE802154,
        }
    }
}
...
pub struct Config {
    ...
    /// CRC skip address
    pub crc_skip_address_mode: CrcSkipAddressMode,
    /// CRC polynomial
...
impl Default for Config {
    fn default() -> Self {
        Self {
            ...
            crc_skip_address_mode: CrcSkipAddressMode::Include,
...
impl<'d, T: Instance> Radio<'d, T> {
    pub fn new(
        ...
        r.mode.write(|w| match config.mode {
            Mode::Ble1Mbit => w.mode().ble_1mbit(),
            Mode::Ble2Mbit => w.mode().ble_2mbit(),
            Mode::Nrf1Mbit => w.mode().nrf_1mbit(),
            Mode::Nrf2Mbit => w.mode().nrf_2mbit(),
            Mode::Ieee250Kbit => w.mode().ieee802154_250kbit(),
        });

        // FREQUENCY
        // FREQUENCY: [0..100] freq = 2400 MHz + freq
        r.frequency
            .write(|w| unsafe { w.frequency().bits((u16::from(config.frequency) - 2400) as u8) });
        ...
        r.pcnf0.write(|w| unsafe {
            w.lflen()
                ...
                .plen()
                .variant(config.preamble_length.into())
        });
        ...
        r.crccnf.write(|w| {
            w.len()
                ...
                .skipaddr()
                .variant(config.crc_skip_address_mode.into())
        });
